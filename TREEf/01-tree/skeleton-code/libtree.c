...
tree_print_recurse(struct fileinfo finfo)
{
  ...
  int is_dir = S_ISDIR(finfo.st.st_mode);
  /* TODO: implement dirsonly functionality here */
  
  if(opts.dirsonly == 0 || is_dir){
    
    /* TODO: print indentation */
    for(size_t i = 0; i < opts.indent * depth; i++){
     printf(" ");
    }
...
print_path_info(struct fileinfo finfo)
{
  ...
  if (opts.perms) {
    char *mdstr = mode_string(finfo.st.st_mode);
    if (printf("%c%s", sep, mdstr) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.user) {
    /*  Hint: getpwuid(3) */
   struct passwd *pw = getpwuid(finfo.st.st_uid);
        if (printf("%c%s", sep, pw->pw_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.group) {
    /*  Hint: getgrgid(3) */
    struct group *gr = getgrgid(finfo.st.st_gid);
    if (printf("%c%s", sep, gr->gr_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.size) {
    /*  Hint: stat.h(0p) */
    intmax_t sz = finfo.st.st_size;
    if (printf("%c%jd", sep, sz) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (sep != '[')
